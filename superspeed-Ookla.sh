...
ensure_cmd() {
    ...
    if ! command -v "$cmd" &>/dev/null; then
        ...
        local apt_out
        apt_out=$(apt install -y "$pkg" 2>&1) || true
        # 验证安装
        if ! command -v "$cmd" &>/dev/null; then
            echo -e "${RED}[ERROR] ${name} 安装失败${PLAIN}"
            echo "$apt_out" | tail -5
            echo -e "${RED}请先运行: apt --fix-broken install -y 后再试${PLAIN}"
            exit 1
        fi
    fi
}
...
init_deps() {
    ...
    if ! command -v speedtest &>/dev/null; then
        ...
        apt install -y speedtest >/dev/null 2>&1 || true
        if ! command -v speedtest &>/dev/null; then
            echo -e "${RED}[ERROR] Speedtest CLI 安装失败${PLAIN}"
            exit 1
        fi
        speedtest --accept-license >/dev/null 2>&1 || true
    fi
}
...
init_nodes() {
    ...
    if ! command -v jq &>/dev/null; then
        echo -e "${RED}[ERROR] jq 未安装，无法解析节点库${PLAIN}"
        exit 1
    fi
    ...
    if echo "$raw" | jq '.' > "$NODES_JSON" 2>/dev/null; then
        ...
        # 删除无效文件，回退本地缓存
        rm -f "$NODES_JSON"
        # 注意：这里检查的是「旧的缓存」—— 需用另一个文件名或先备份
        # 简单处理：如果 nodes.json 刚被删了，尝试从备份恢复（如果之前存过）
        if [ -f "${NODES_JSON}.bak" ] && [ -s "${NODES_JSON}.bak" ]; then
            cp "${NODES_JSON}.bak" "$NODES_JSON"
            echo -e "${YELLOW}[INFO] 使用本地缓存${PLAIN}"
        else
            echo -e "${RED}[ERROR] 无缓存可用，退出${PLAIN}"
            exit 1
...
run_one_type() {
    ...
    while read -r item; do
        ...
        if echo "$result" | grep -qi "Too many requests\|Limit reached"; then
...
test_multi_id() {
    ...
    case $mode in
        ...
        4)
            ...
            echo ""
            printf "${YELLOW}%-4s %-24s %8s %10s${PLAIN}\n" "序号" "节点" "ID" "状态"
            echo -e "${YELLOW}----------------------------------------------${PLAIN}"
